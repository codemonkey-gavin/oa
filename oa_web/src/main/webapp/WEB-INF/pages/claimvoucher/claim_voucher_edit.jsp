    ...
    <div class="tray tray-center">
        ...
        <div class="admin-form theme-primary mw1000 center-block" style="padding-bottom: 175px;">
            <div class="panel heading-border">
                <form:form id="admin-form" name="editForm" action="/claim_voucher/edit" modelAttribute="info" method="post">
                    <div class="panel-body bg-light">
                        ...
                        <div class="section row" id="items">
                           <c:forEach var="item" items="${info.items}" varStatus="vs">
                               <div>
                                   <form:hidden path="items[${vs.index}].id"/>
                                   <form:hidden path="items[${vs.index}].claimVoucherId"/>
                                   <div class="col-md-3">
                                       <label for="items[${vs.index}].item" class="field prepend-icon">
                                           <form:select path="items[${vs.index}].item" items="${items}" class="gui-input" placeholder="花销类型..."/>
                                       </label>
                                   </div>
                                   <div class="col-md-3">
                                       <label for="items[${vs.index}].amount" class="field prepend-icon">
                                           <form:input path="items[${vs.index}].amount" class="gui-input money" placeholder="金额..."/>
                                           <label for="items[${vs.index}].amount" class="field-icon">
                                               <i class="fa fa-lock"></i>
                                           </label>
                                       </label>
                                   </div>
                                   <div class="col-md-5">
                                       <label for="items[${vs.index}].comment" class="field prepend-icon">
                                           <form:input path="items[${vs.index}].comment" class="gui-input" placeholder="备注..."/>
                                           <label for="items[${vs.index}].comment" class="field-icon">
                                               <i class="fa fa-lock"></i>
                                           </label>
                                       </label>
                                   </div>
